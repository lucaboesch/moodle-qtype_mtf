...
class qtype_mtf_renderer extends qtype_renderer {
    ...
    public function formulation_and_controls(question_attempt $qa,
        ...
        foreach ($question->get_order($qa) as $key => $rowid) {
            ...
            foreach ($question->columns as $column) {
                ...
                $radio = $this->radiobutton($buttonname, $column->number, $ischecked, $isreadonly, $buttonid, $datacol, $datamulti, $singleormulti, $qtype_mtf_id);
     ...
     * @return string
     */
    protected static function radiobutton($name, $value, $checked, $readonly, $id = '', $datacol = '', $datamulti = '', $singleormulti = 2, $qtype_mtf_id = '') {
		...
		if ($singleormulti >= 2){			
			...
			$result .= '<input type="hidden" id="hidden_'. $id .'" name="'. $name .'" value="2" data-hiddenmtf="' . $qtype_mtf_id .  '" disabled="disabled">';	 
