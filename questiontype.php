...
/**
 * Question type class for qtype_mtf.
 *
 * @package     qtype_mtf
...
class qtype_mtf extends question_type {
    ...
    public function save_question_options($question) {
        ...
        for ($i = 1; $i <= $options->numberofrows; ++$i) {
            ...
            $row->optiontext = $this->import_or_save_files($optiondata, $context, 'qtype_mtf', 'optiontext', $row->id);
            $row->optiontextformat = $question->option[$i - 1]['format'];

            $optionfeedback = $question->feedback[$i - 1];
            $row->optionfeedback = $this->import_or_save_files($optionfeedback, $context, 'qtype_mtf', 'feedbacktext', $row->id);
    ...
    /**
     * Get different numbering styles
    ...
    protected function move_files_in_options_and_feedback($questionid, $oldcontextid, $newcontextid, $answerstoo = false) {
        ...
        foreach ($rowids as $rowid => $notused) {
            ...
            $fs->move_area_files_to_new_context($oldcontextid, $newcontextid, 'qtype_mtf', 'feedbacktext', $rowid);
        }
    }
